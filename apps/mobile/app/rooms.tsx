...
export default function RoomsScreen() {
  ...
  const [joinModal, setJoinModal] = useState<{ roomId: string; needsPassword: boolean } | null>(null);
  ...
  return (
    <View style={styles.container}>
      ...
      {/* 房间列表 */}
      {loading ? (
        <ActivityIndicator style={{ marginTop: 40 }} color="#4ade80" size="large" />
      ) : (
        <FlatList
          data={rooms}
...
const styles = StyleSheet.create({
  ...
  modalTitle: { color: '#e5e7eb', fontSize: 18, fontWeight: 'bold', marginBottom: 16 },
  input: {
