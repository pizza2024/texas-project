...
function CardDisplay({ card }: { card: string }) {
  ...
  return (
    <View style={[styles.card, { backgroundColor: '#fff' }]}>
      <Text style={[styles.cardRank, { color: isRed ? '#dc2626' : '#1e293b' }]}>{rank}</Text>
      <Text style={[styles.cardSuit, { color: isRed ? '#dc2626' : '#1e293b' }]}>{suitSymbol[suit] ?? suit}</Text>
    </View>
  );
}

function HandCard({ hand, userId }: { hand: HandHistoryEntry; userId: string }) {
      ...
      {hand.players.map((player) => (
        ...
        >
          <View style={styles.playerInfo}>
            <Text style={[styles.playerName, player.id === userId && styles.myPlayerName]}>
          ...
          <View style={styles.playerProfit}>
            <Text
              style={[
                styles.profitText,
                player.winAmount > 0 ? styles.profitGreen : player.netProfit < 0 ? styles.profitRed : styles.profitGray,
              ]}
            >
              {player.winAmount > 0 ? `+${player.winAmount.toFixed(2)}` : player.netProfit !== 0 ? player.netProfit.toFixed(2) : '-'}
            </Text>
          </View>
        </View>
      ))}
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <View style={styles.centered}>
          ...
          <TouchableOpacity style={styles.retryBtn} onPress={() => { setError(null); setLoading(true); void loadHands(0); }}>
        ...
        <FlatList
          ...
          ListFooterComponent={hasMore ? <ActivityIndicator color="#4ade80" style={{ marginVertical: 20 }} /> : null}
        />
      )}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  retryBtn: { backgroundColor: 'rgba(74,222,128,0.15)', paddingHorizontal: 24, paddingVertical: 10, borderRadius: 8 },
  ...
  handHeader: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 10 },
  handRoom: { color: '#e5e7eb', fontSize: 15, fontWeight: 'bold' },
  handDate: { color: '#6b7280', fontSize: 12 },
  cardsRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap' },
  ...
  cardQuestion: { color: 'rgba(255,255,255,0.3)', fontSize: 16, fontWeight: 'bold' },
  ...
  myPlayerRow: { backgroundColor: 'rgba(74,222,128,0.08)', borderWidth: 1, borderColor: 'rgba(74,222,128,0.2)' },
  winnerRow: { backgroundColor: 'rgba(74,222,128,0.12)', borderWidth: 1, borderColor: 'rgba(74,222,128,0.4)' },
  playerInfo: { flex: 1 },
