...
export default function RegisterScreen() {
  ...
  const handleRegister = async () => {
    ...
    if (!username.trim()) {
      Alert.alert(
        t("common.confirm"),
        t("auth.inputRequired", { field: t("auth.username") }),
      );
      return;
    }
    ...
    if (!nickname.trim()) {
      Alert.alert(
        t("common.confirm"),
        t("auth.inputRequired", { field: t("auth.nickname") }),
      );
      return;
    }
    if (!password.trim()) {
      Alert.alert(
        t("common.confirm"),
        t("auth.inputRequired", { field: t("auth.password") }),
      );
      return;
    }
    ...
    try {
      ...
      Alert.alert(t("auth.registerSuccess"), t("auth.registerSuccessMsg"), [
        { text: t("auth.goLogin"), onPress: () => router.replace("/login") },
      ]);
    } catch (error: unknown) {
      const err = error as { response?: { data?: { message?: string } } };
      const message =
        err?.response?.data?.message ?? t("auth.registerFailedMsg");
