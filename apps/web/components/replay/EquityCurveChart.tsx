...
export function EquityCurveChart({ data, currentStage: _currentStage }: EquityCurveChartProps) {
  ...
  for (const stage of stages) {
    ...
    const activePlayers = data.players.filter(p => {
      ...
      const timelineNode = data.timeline.find(n =>
        n.playerId === p.id && n.stage.toUpperCase() === stage
      );
    ...
    if (activePlayers.length === 0) {
      ...
      for (const p of data.players) {
        ...
        if (!isActive) {
          ...
          const winners = activePlayers.filter(ap => (playerHandStrengths.get(ap.id) ?? 0) === maxStrength);
  ...
  return (
    <div
      ...
      style={{ borderColor: 'rgba(245,158,11,0.1)', background: 'rgba(0,0,0,0.2)' }}
    >
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs font-bold uppercase tracking-widest" style={{ color: 'rgba(245,158,11,0.7)' }}>
          Equity Curve
        </span>
      </div>
        ...
        <LineChart data={chartData} margin={{ top: 4, right: 8, bottom: 0, left: -20 }}>
          ...
          <Legend
            ...
            formatter={(value) => {
              ...
              return <span style={{ color: 'rgba(255,255,255,0.7)' }}>{player?.nickname ?? value}</span>;
            }}
          />
