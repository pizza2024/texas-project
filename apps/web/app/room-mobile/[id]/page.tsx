  ...
  if (hidden) {
    return (
      <div
        ...
        style={{
          background:
            "linear-gradient(135deg, #1e3a5f 0%, #0f2540 60%, #0a1a30 100%)",
          border: "1px solid rgba(96,165,250,0.4)",
          boxShadow:
            "0 4px 16px rgba(0,0,0,0.5), inset 0 0 20px rgba(96,165,250,0.05)",
...
const pageBg = {
  background: "linear-gradient(180deg, #050d08 0%, #020405 100%)",
};
...
export default function MobileRoomPage() {
  ...
  useEffect(() => {
    ...
    socket.on("disconnect", () => {
      ...
      if (!t || isTokenExpired(t, 1000)) router.replace("/login");
    });
    socket.on("room_full", () => router.replace("/rooms"));
    socket.on("error", (msg: string) => {
      if (msg === "Room not found") router.replace("/rooms");
    });
    return () => {
      socket.off("connect");
  ...
  // ── Auto-action ─────────────────────────────────────────────────────────
  const doAutoAction = useCallback(
    (currentTable: typeof table) => {
      if (!currentTable) return;
      const socket = getSocket(getStoredToken()!);
      if (!socket) return;
      const myP = currentTable.players.find(
        (p: Player | null) => p?.id === myUserId,
      ) as Player | null;
      const callAmt = myP
        ? Math.max(0, (currentTable.currentBet ?? 0) - myP.bet)
        : 0;
  ...
  useEffect(() => {
    ...
    if (previousTable) {
      ...
      table.players.forEach((player, idx) => {
        ...
        const playerCenter = getElementCenter(
          playerNodeRefs.current[player.id] ?? null,
        );
      ...
      if (
        ...
        const winners = table.lastHandResult.filter(
          (entry: HandResultEntry) => entry.winAmount > 0,
        );
        const myResult = table.lastHandResult.find(
          (entry: HandResultEntry) => entry.playerId === myUserId,
        );
        ...
        winners.forEach((entry: HandResultEntry) => {
          ...
          const potCenter = getElementCenter(potRef.current);
          const playerCenter = getElementCenter(
            playerNodeRefs.current[entry.playerId] ?? null,
          );
          if (!potCenter || !playerCenter) return;
  ...
  if (!table)
    return (
      ...
      >
        ...
        <p className="text-sm tracking-[0.3em] uppercase font-semibold text-yellow-500/70">
          Connecting…
        </p>
  ...
  const isFoldWinWinner =
    isFoldWinSettlement &&
    winners.some((w: HandResultEntry) => w.playerId === myUserId);
  ...
  return (
    <div
      className="min-h-screen text-white relative flex flex-col pb-52"
      style={pageBg}
    >
      {/* ── Header ── */}
      <div
      ...
      {/* ── Stage banner ── */}
      <div
        className="px-4 py-1.5 text-center text-xs tracking-widest uppercase shrink-0"
        style={{
      ...
      <div className="relative shrink-0">
        ...
        >
          {players.map((p) => {
            const active =
              table.activePlayerIndex >= 0 &&
              table.players[table.activePlayerIndex]?.id === p.id;
              ...
              >
                <div
                  ...
                  style={{
                    background: folded
                      ? "rgba(80,80,80,0.5)"
                      : "linear-gradient(135deg, #1a3a2a, #0d2015)",
                    border: winner
                ...
                <span
                  className="text-[9px] font-bold truncate max-w-[48px]"
                  style={{
                    color:
                      p.id === myUserId ? "#fcd34d" : "rgba(255,255,255,0.7)",
                  }}
                >
                ...
                {folded && (
                  <span className="text-[7px] text-gray-500">FOLD</span>
                )}
              </div>
            );
          })}
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(90deg, rgba(2,4,6,0.6) 0%, transparent 100%)",
          }}
        />
        <div
          ...
          style={{
            background:
              "linear-gradient(270deg, rgba(2,4,6,0.6) 0%, transparent 100%)",
          }}
        />
      </div>
          ...
          <div className="flex gap-3">
            {myPlayer && myPlayer.cards.length > 0
              ? myPlayer.cards.map((c, i) => (
                  <CardDisplay key={i} card={c} large />
                ))
              : [
                  <div
                    key={0}
                    className="w-16 h-[5.5rem] rounded-xl"
                    style={{
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(180deg, rgba(2,4,6,0) 0%, rgba(2,4,6,0.97) 40%, rgba(2,4,6,1) 100%)",
              ...
              <div className="text-center">
                ...
                >
                  {winners
                    .map(
                      (w: HandResultEntry) =>
                        `${w.nickname} wins $${fmt(w.winAmount)}`,
                    )
                    .join(" / ")}
                </p>
              </div>
            ...
            {showFoldWinChoice ? (
              <div className="w-full flex flex-col items-center gap-2">
                <span
                  ...
                  style={{ color: "rgba(251,191,36,0.82)" }}
                >
                  Show or Muck
                </span>
            ...
            {isMyTurn && (
              <div className="flex gap-2">
                ...
                <button
                  onClick={() =>
                    setRaiseAmount(
                      Math.min(minRaise, myPlayer?.stack ?? minRaise),
                    )
                  }
                ...
                <button
                  ...
                  onClick={() =>
                    setRaiseAmount(
                      Math.max(
                        minRaise,
                        raiseAmount - (myPlayer?.stack ?? minRaise) / 10,
                      ),
                    )
                  }
