...
import type { FriendStatusUpdatePayload, FriendRequestReceivedPayload } from '@texas/shared';
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h2 className="text-lg font-black tracking-wide" style={{ color: '#6ee7b7' }}>
              {t('friends.addFriendTitle')}
            </h2>
          </div>
          <button onClick={onClose} className="ml-auto text-gray-500 hover:text-gray-300 transition-colors text-lg leading-none">
            ✕
          </button>
        </div>
...
function DeleteFriendModal({ friendId, friendNickname, onClose, onDeleted }: DeleteModalProps) {
  const { t } = useTranslation();
  const [loading, setLoading] = useState(false);
  const overlayRef = useRef<HTMLDivElement>(null);
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background: 'linear-gradient(160deg, rgba(20,10,10,0.99) 0%, rgba(10,5,5,1) 100%)',
      ...
      >
        <div className="text-center">
          ...
          <p className="text-xs mt-1 text-gray-500">{t('friends.deleteConfirmMsg')}</p>
        </div>
        <p className="text-center text-sm font-bold text-gray-300">{friendNickname}</p>
...
export default function FriendsPage() {
  ...
  /* ── Load friends ── */
  const loadFriends = useCallback(async (searchQuery = '') => {
    try {
      const params: Record<string, string> = {};
      if (searchQuery) params.search = searchQuery;
      const res = await api.get<{ data: FriendItem[] }>('/friends', { params });
  ...
  const loadRequests = useCallback(async () => {
    try {
      const res = await api.get<{ data: FriendRequest[] }>('/friends/requests', {
        params: { status: 'PENDING' },
      });
      setRequests(res.data.data ?? res.data);
      setPendingCount(Array.isArray(res.data.data ?? res.data) ? (res.data.data ?? res.data).length : 0);
    } catch {
      // silent
    }
  }, []);
      ...
      setFriends(prev =>
        prev.map(f =>
          f.user.id === data.friendUserId
            ? { ...f, user: { ...f.user, status: data.online ? 'ONLINE' as const : 'OFFLINE' as const } }
            : f
        )
      );
    });
  ...
  const handleAccept = async (requestId: string) => {
    ...
    finally { setActioningIds(prev => { const s = new Set(prev); s.delete(requestId); return s; }); }
  };
      ...
      <div className="relative z-10 max-w-lg mx-auto px-4 pt-6 pb-24">
        ...
        <header className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-black tracking-[0.08em] uppercase"
                style={{
          ...
          {(['friends', 'requests'] as Tab[]).map((t_) => (
            <button
              ...
              style={{
                ...
                border: tab === t_ ? '1px solid rgba(16,185,129,0.3)' : '1px solid transparent',
              }}
            >
        ...
        {loading && (
          <div className="flex justify-center py-16">
            <span className="text-yellow-600/60 text-sm tracking-[0.3em] animate-pulse uppercase">{t('common.loading')}</span>
          </div>
        )}
          ...
          <>
            {friends.length === 0 ? (
              <div className="flex flex-col items-center py-16 gap-3">
                ...
                <p className="text-sm font-bold" style={{ color: 'rgba(245,158,11,0.7)' }}>{t('friends.noFriends')}</p>
                <p className="text-xs text-gray-600 text-center max-w-[220px]">{t('friends.noFriendsHint')}</p>
              </div>
            ) : (
              <div className="space-y-2">
                {friends.map((f) => (
                  ...
                  >
                    <UserAvatar userId={f.user.id} avatar={f.user.avatar} size={44} />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-bold text-gray-200 truncate">{f.user.nickname}</p>
                      <div className="mt-0.5">
                        <StatusBadge status={f.user.status} />
                      </div>
                    </div>
                    <button
                      onClick={() => setDeleteTarget({ id: f.id, nickname: f.user.nickname })}
          ...
          <>
            {requests.length === 0 ? (
              <div className="flex flex-col items-center py-16 gap-3">
                ...
                <p className="text-sm font-bold" style={{ color: 'rgba(245,158,11,0.7)' }}>{t('friends.noRequests')}</p>
                <p className="text-xs text-gray-600 text-center max-w-[220px]">{t('friends.noRequestsHint')}</p>
              </div>
            ) : (
              <div className="space-y-2">
                {requests.map((req) => (
                  ...
                  >
                    <UserAvatar userId={req.requester.id} avatar={req.requester.avatar} size={44} />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-bold text-gray-200 truncate">{req.requester.nickname}</p>
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        style={{ background: 'rgba(16,185,129,0.8)', color: '#ecfdf5' }}
                      >
                        {t('friends.accept')}
                      </button>
                      <button
