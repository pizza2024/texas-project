  ...
  return (
    ...
    >
      ...
      <div className="text-center">
        {isSettlement ? (
          <div className="flex flex-col items-center gap-1">
            <span
              className="text-[10px] tracking-[0.26em] uppercase font-semibold"
              style={{ color: "rgba(251,191,36,0.78)" }}
            >
              {t("room.settling")}
            </span>
            <span
              className={
                isUrgentCountdown
                  ? "countdown-badge countdown-badge-urgent"
                  : "countdown-badge"
              }
              style={{
                color: isUrgentCountdown ? "#fee2e2" : "#fef3c7",
                borderColor: isUrgentCountdown
          ...
          <div className="flex flex-col items-center gap-1">
            <span
              className="text-[10px] tracking-[0.26em] uppercase font-semibold"
              style={{ color: "rgba(74,222,128,0.82)" }}
            >
          ...
          <div className="flex flex-col items-center gap-1">
            ...
            >
              {isMyTurn
                ? t("room.yourTurn")
                : t("room.waitingPlayer", { nickname: activePlayer.nickname })}
            </span>
            {actionCountdown > 0 && (
              <span
                className={
      ...
      <div
        ...
        style={{ color: "rgba(107,114,128,0.5)", minWidth: "60px" }}
      >
        <span
          className={`tracking-widest uppercase ${activeCountdown > 0 ? "countdown-text-glow" : ""}`}
        >
          {activeCountdown > 0
            ? `${countdownLabel} ${activeCountdown}s`
            : table.currentStage}
        </span>
      </div>
    </div>
