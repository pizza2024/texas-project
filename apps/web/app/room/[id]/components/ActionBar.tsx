...
export function ActionBar({
  ...
  // P2-UX-002: Auto-focus raise input + sync default value when it's player's turn
  useEffect(() => {
    if (isMyTurn && raiseInputRef.current) {
  ...
  return (
    <div
      ...
      style={{
        background:
          "linear-gradient(180deg, rgba(2,4,6,0) 0%, rgba(2,4,6,0.96) 40%, rgba(2,4,6,1) 100%)",
        ...
        <div className="flex flex-col items-center gap-2">
          <div
            className={`px-4 py-2 rounded-full text-[11px] font-bold tracking-[0.12em] uppercase ${
              isMyTurn && isUrgentCountdown
                ? "action-timeout-banner action-timeout-banner-urgent"
          ...
          >
            {isMyTurn
              ? t("room.autoActionLabel", {
                  seconds: actionCountdown,
                  action: canCheck ? t("room.autoCheck") : t("room.autoFold"),
                })
          ...
          <div className="flex justify-center gap-3 items-center flex-wrap">
            ...
            {isMyTurn &&
              ...
              !table.straddle && (
                <Button
                  ...
                  style={{
                    background:
                      "linear-gradient(135deg, #854d0e 0%, #a16207 30%, #ca8a04 65%, #facc15 100%)",
                    color: "#000",
                    border: "none",
            ...
            <Button
              ...
              className={`h-11 px-5 font-black tracking-wider text-xs uppercase rounded-lg transition-opacity disabled:opacity-30 ${
                !canCheck && isMyTurn && isUrgentCountdown
                  ? "action-auto-button action-auto-button-urgent"
                  : ""
              }`}
              style={{
                background: isMyTurn
            ...
            >
              {isMyTurn && !canCheck
                ? t("room.foldCountdown", { seconds: actionCountdown })
                : isMyTurn && canCheck
                  ? `${t("room.fold")} ·${actionCountdown}s`
                  : t("room.fold")}
            </Button>
            ...
            {canCheck ? (
              ...
              >
                {isMyTurn
                  ? t("room.checkCountdown", { seconds: actionCountdown })
                  : t("room.check")}
              </Button>
            ) : (
              <div className="flex flex-col items-center gap-1">
                ...
                >
                  {isMyTurn
                    ? `${t("room.call", { amount: callAmount })} ·${actionCountdown}s`
            ...
            {isMyTurn && (
              <div className="flex items-center gap-1">
                ...
                <Button
                  ...
                  disabled={
                    !isMyTurn ||
                    effectiveMinRaiseTo > myPlayerStack ||
                    effectiveMinRaiseTo < minRaiseTo
                  }
            ...
            {/* P2-UX-003: Independent large ALL-IN golden button — WSOP style */}
            {isMyTurn && myPlayerStack > 0 && (
              <Button
                onClick={() =>
                  onRequestAllIn
                    ? onRequestAllIn(myPlayerStack)
                ...
                className="h-14 px-8 font-black tracking-wider text-sm uppercase rounded-xl transition-all hover:scale-105 active:scale-100"
                style={{
                  background:
                    "linear-gradient(135deg, #854d0e 0%, #a16207 25%, #ca8a04 50%, #facc15 75%, #fde047 100%)",
                  color: "#000",
                  border: "2px solid rgba(250,204,21,0.7)",
                  boxShadow:
                    "0 0 28px rgba(250,204,21,0.35), 0 4px 12px rgba(0,0,0,0.4)",
                }}
              >
                <div className="flex flex-col items-center leading-none">
                  <span>{t("room.allIn")}</span>
            ...
            <div className="flex items-center gap-2">
              <Input
                ...
                value={Math.min(
                  isMyTurn && raiseAmount === 0 ? minRaiseTo : raiseAmount,
                  myPlayerStack,
                )}
                min={minRaiseTo}
                max={myPlayerStack}
                onChange={(e) =>
                  setRaiseAmount(
                    Math.min(Number(e.target.value), myPlayerStack),
                  )
                }
                disabled={!isMyTurn}
              />
