...
import { Search, ChevronLeft, ChevronRight, UserX, UserCheck, PlusCircle } from 'lucide-react';
...
export default function UsersPage() {
  ...
  const [dialog, setDialog] = useState<{ type: string; user: any } | null>(null);
    ...
    <AdminLayout>
      <div className="p-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <span className="text-slate-400 text-sm">共 {data?.total ?? 0} 名用户</span>
        </div>
        ...
        <form onSubmit={handleSearch} className="flex gap-3 mb-6">
          ...
          <button type="submit" className="bg-indigo-600 hover:bg-indigo-500 text-white text-sm px-4 py-2.5 rounded-lg transition-colors">
            搜索
          </button>
        </form>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} className="text-center py-12 text-slate-500">暂无数据</td></tr>
              ) : (
                data?.data?.map((user: any) => (
                  <tr key={user.id} className="border-b border-[#1e2535] hover:bg-white/2 transition-colors">
                    <td className="px-4 py-3 text-white font-medium">
                      <Link href={`/users/${user.id}`} className="hover:text-indigo-400 transition-colors">
                        {user.nickname}
                      </Link>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => { setBalanceAmount(''); setDialog({ type: 'balance', user }); }}
        ...
        {dialog?.type === 'ban' && (
          <ConfirmDialog
            ...
            confirmLabel="封禁"
            danger
            onConfirm={async () => { await banUser(dialog.user.id); fetchUsers(); }}
        ...
        {dialog?.type === 'unban' && (
          <ConfirmDialog
            ...
            confirmLabel="解封"
            onConfirm={async () => { await unbanUser(dialog.user.id); fetchUsers(); }}
            onClose={() => setDialog(null)}
          />
        )}
        {dialog?.type === 'balance' && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-[#161b27] border border-[#1e2535] rounded-2xl p-6 max-w-sm w-full">
              ...
              <p className="text-slate-400 text-sm mb-4">用户：{dialog.user.nickname}（当前余额：{dialog.user.coinBalance}）</p>
              <input
                type="number"
                value={balanceAmount}
              ...
              <div className="flex gap-3 justify-end">
                <button onClick={() => setDialog(null)} className="px-4 py-2 text-sm text-slate-400 hover:bg-white/5 rounded-lg">取消</button>
                <button
                  onClick={async () => {
                    const amt = parseFloat(balanceAmount);
                    if (!isNaN(amt)) { await adjustBalance(dialog.user.id, amt, 'Admin adjustment'); fetchUsers(); setDialog(null); }
                  }}
