    ...
    <AdminLayout>
      <div className="p-8">
        <div className="flex items-center gap-3 mb-6">
          <Link href="/rooms" className="text-slate-400 hover:text-white"><ArrowLeft size={20} /></Link>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div className="xl:col-span-1 bg-[#161b27] border border-[#1e2535] rounded-xl p-6 space-y-4">
            ...
            {room.password && <p className="text-yellow-400 text-sm">🔒 私有房间（已设置密码）</p>}

            <div className="space-y-3 pt-2 border-t border-[#1e2535] text-sm">
              {[
                { label: '盲注', value: `${room.blindSmall} / ${room.blindBig}` },
                { label: '最大人数', value: room.maxPlayers },
                { label: '最低买入', value: room.minBuyIn.toLocaleString() },
                { label: '创建时间', value: new Date(room.createdAt).toLocaleDateString('zh-CN') },
            ...
            <div className="space-y-2 pt-2 border-t border-[#1e2535]">
              <button
                onClick={async () => { await toggleRoomMaintenance(id); load(); }}
                  ...
                  room.tables?.map((table: Table) => (
                    <tr key={table.id} className="border-b border-[#1e2535]">
                      ...
                      <td className="px-4 py-3">
                        <Badge variant={table.state === 'WAITING' ? 'default' : 'info'}>{table.state}</Badge>
                      </td>
                      <td className="px-4 py-3 text-slate-500 text-xs">
                        {table.snapshotUpdatedAt ? new Date(table.snapshotUpdatedAt).toLocaleString('zh-CN') : '-'}
                      </td>
                    </tr>
                  ))
                )}
        ...
        {deleteDialog && (
          <ConfirmDialog
            ...
            confirmLabel="删除"
            danger
            onConfirm={async () => { await deleteRoom(id); router.push('/rooms'); }}
