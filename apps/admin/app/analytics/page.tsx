...
export default function AnalyticsPage() {
  ...
  if (loading)
    return (
      <AdminLayout>
        <div className="p-4 sm:p-6 lg:p-8 text-slate-400">加载中...</div>
      </AdminLayout>
    );
      ...
      <div className="p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            {
              label: "总牌局数",
              value: handsStats?.total ?? 0,
              color: "text-indigo-400",
            },
            {
              label: "平均底池",
              value: (handsStats?.avgPot ?? 0).toFixed(1),
              color: "text-yellow-400",
            },
            {
              label: "总底池",
              value: (handsStats?.totalPot ?? 0).toLocaleString(),
              color: "text-green-400",
            },
          ].map(({ label, value, color }) => (
            <div
              key={label}
              className="bg-[#161b27] border border-[#1e2535] rounded-xl p-5"
            >
            ...
            {hotRooms.length === 0 ? (
              ...
              hotRooms.map((room, idx) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <span
                      className={`text-xs ${room.status === "ACTIVE" ? "text-green-400" : "text-yellow-400"}`}
                    >
