...
function makePlayer(overrides: Partial<Parameters<typeof createTable>[2] extends infer T ? T : never> = {}): any {
  return {
    sub: 'player-1',
    username: 'Player1',
    nickname: 'Player One',
...
function setupHeadsUp(table: Table): void {
  const p1 = makePlayer({ sub: 'p1', nickname: 'Alice', stack: 1000, status: PlayerStatus.ACTIVE });
  const p2 = makePlayer({ sub: 'p2', nickname: 'Bob', stack: 1000, status: PlayerStatus.ACTIVE });
...
describe('Sit-out Detection', () => {
  ...
  describe('checkAndAutoFoldSittingOut', () => {
    ...
    it('auto-folds a SITOUT player and sets lastSitoutAutoFold', () => {
      ...
      expect(table.checkAndAutoFoldSittingOut()).toBe(false);
      expect(table.lastSitoutAutoFold).toEqual({ playerId: 'p2', seatIndex: bobIndex });
    ...
    it('auto-folds a SITOUT player and advances to next active player', () => {
      ...
      table.addPlayer(makePlayer({ sub: 'p1', nickname: 'Alice', stack: 1000 }), 1000);
      table.addPlayer(makePlayer({ sub: 'p2', nickname: 'Bob', stack: 1000 }), 1000);
      table.addPlayer(makePlayer({ sub: 'p3', nickname: 'Charlie', stack: 1000 }), 1000);
      table.startHand();
      ...
      expect(table.lastSitoutAutoFold).toEqual({ playerId: 'p3', seatIndex: charlieIndex });
      expect(charlie.status).toBe(PlayerStatus.FOLD);
      // After folding Charlie, Alice (SB, ACTIVE) should be next
      expect(table.activePlayerIndex).toBe(table.players.findIndex(p => p?.id === 'p1'));
    });
  ...
  describe('sittingOutTimeout config', () => {
    ...
    it('is restored via fromSnapshot', () => {
      ...
      const snapshot = table.toSnapshot();
      const restored = Table.fromSnapshot(snapshot, 6, 5, 10, 1000, null, { sittingOutTimeout: 25000 });
