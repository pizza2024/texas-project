 ...
 */
function fillCommunity(existing: string[], usedCards: Set<string>): string[] {
...
export function calculateEquity(
  ...
  for (let i = 0; i < simulations; i++) {
    ...
    for (const oppCards of opponentHoleCards) {
      ...
      if (
        !bestOpponentScore ||
        compareScores(oppScore, bestOpponentScore) > 0
      ) {
        bestOpponentScore = oppScore;
      }
    }

    const compared = bestOpponentScore
      ? compareScores(myScore, bestOpponentScore)
      : 1;
    if (compared > 0) {
      wins++;
    } else if (compared === 0) {
