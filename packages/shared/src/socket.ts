...
export interface ServerToClientEvents {
  ...
  /** Matchmaking lobby timed out (e.g. Blast lobby didn't fill within timeout) */
...
export function setRejoinAvailableHandler(
  h: (data: { roomId: string }) => void,
) {
  rejoinAvailableHandler = h;
}

export function setDepositConfirmedHandler(
  h: (data: DepositConfirmedPayload) => void,
) {
  depositConfirmedHandler = h;
}

export function setFriendStatusUpdateHandler(
  h: (data: FriendStatusUpdatePayload) => void,
) {
  friendStatusUpdateHandler = h;
}

export function setFriendRequestReceivedHandler(
  h: (data: FriendRequestReceivedPayload) => void,
) {
...
export function getSocket(
  ...
  if (!socket || socketToken !== token) {
    ...
    socket.on("friend_status_update", (data) =>
      friendStatusUpdateHandler?.(data),
    );
