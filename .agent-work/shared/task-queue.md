...
|     |     |     | ID                                              | 任务                                            | 状态   | 备注                                                              |
| --- | --- | --- | --------------------------------------------- | --------------------------------------------- | ------ | ---------------------------------------------------------------- |
|     |     |     | P0-DEPOSIT-E2E-001                           | `deposit.e2e.spec.ts` mock 缺失 `findFirst`   | ✅ 已修复 | commit `develop` HEAD — 添加 `findFirst` mock，`resetAllMocks`→`clearAllMocks`，更新7个测试，1个跳过 |
...
| P1-TEST-003         | Bot prefix 碰撞导致 `coinBalance` 从不同步           | ✅ 已验证无问题 | bot\_ UUID 唯一，setBalances 已跳过 BOT_ID_PREFIX 用户                                               |
...
| P2-NOTIFY-001 | 站内通知中心 | ✅ Phase 1+2+3 完成 | Phase 1+2: commit 3386592; Phase 3: commit HEAD — DND settings + quiet hours + push/email toggles + WS push suppression |
...
| P2-WEB3-001   | `EXPLORER_URL` 默认 Sepolia Etherscan，但充值使用 TRON | ✅ 已关闭 | 项目使用 ETH (ethers.js)，非 TRON — 无需修复；mobile withdraw.tsx 已改用 env var |

## P2 — 新发现（第22轮）

| ID                          | 任务                                                              | 状态      | 备注                                                                 |
| --------------------------- | ----------------------------------------------------------------- | --------- | -------------------------------------------------------------------- |
| P2-DEPOSIT-BONUS-TX         | 首次充值 bonus 逻辑在 `$transaction` 外执行                       | ✅ 已修复 | deposit.service.ts — hasReceivedFirstDepositBonus 更新移入 atomic tx |
| P2-DEPOSIT-WS-NULL          | `wsManager.emitToUser` 无 server 时静默 no-op                     | ✅ 已修复 | deposit.service.ts — 添加 getServer() 检查 + warn 日志               |
| P2-DEPOSIT-FIRST-BONUS-RACE | `hasReceivedFirstDepositBonus` 检查在事务外，并发充值可能重复触发 | ✅ 已修复 | 同 P2-DEPOSIT-BONUS-TX — atomic tx 内更新 flag 防止竞态              |

## P2 — 新发现（第23轮）

| ID           | 任务                                                        | 状态              | 备注                                                                             |
| ------------ | ----------------------------------------------------------- | ----------------- | -------------------------------------------------------------------------------- |
| P2-TABLE-019 | `rakePercent` 在 `resolveFoldWin` 中未赋值（showdown 中有） | ✅ 已验证无需修复 | table-round.ts:314 — `rakePercent = tierConfig.rate` 已在 commit 8e33987 中修复  |
| P2-TABLE-020 | `calledAllIn` 字段从未被读取 — 废弃代码                     | ✅ 已验证无需修复 | table-player-ops.ts:325-384 读取 `this.table.calledAllIn` — 非废弃代码           |
| P2-TABLE-021 | `attemptStraddle` 中 `calledAllIn = null` 在校验之前重置    | ✅ 已修复         | commit c0083d5 — 重置移至 stage/straddle/activePlayer 校验之后                   |
| P2-TABLE-022 | `leaveCurrentRoom` 两次 wallet 调用非原子                   | ✅ 已修复         | commit c0083d5 — `setBalance + unfreezeBalance` → 原子 `resetBalanceAndUnfreeze` |
| P2-WS-015    | `hasOtherActiveSocketFn` callback 参数冗余（4传2）          | ✅ 已修复         | commit c0083d5 — 闭包直接捕获 `client.id`，传 2 参数给 4-param 函数              |
| P2-WS-016    | `handleEmojiReaction` DB 查询在房间校验前                   | ✅ 已修复         | commit c0083d5 — `getUserCurrentRoomId` 移至 rate limit 之后、DB 查询之前        |

## P2 — 新发现（第25轮）

| ID            | 任务                                                                                                                            | 状态      | 备注                             |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------- | --------- | -------------------------------- |
| P2-TEST-STALE | `table-manager.service.spec.ts:116` 期望 `setBalance` 但实现已改为 `resetBalanceAndUnfreeze`（P2-TABLE-022 fix 后 spec 未同步） | ✅ 已修复 | commit c9301dc — spec 断言已更正 |
...
_最后更新: 2026-04-30 10:45 — Coding 第405轮 — P0-NOTIFY-MIGRATE 迁移脚本已创建 ✅_
...
| ID        | 任务                                                 | 紧迫度 | 状态   | 备注             |
| --------- | ---------------------------------------------------- | ------ | ------ | ---------------- |
| P2-DEPOSIT-UX-001 | 充值成功无动画反馈 | ✅ 已完成 | commit d0a5cd4 — deposit/page.tsx WS handler + framer-motion overlay |
| P2-DEPOSIT-UX-002 | 充值地址簿 | ✅ 已完成 | commit a9b2b86 — label/isDefault/GET-POST-DELETE-PATCH /deposit/addresses |
...
| ID        | 任务                                                 | 紧迫度 | 状态   | 备注             |
| --------- | ---------------------------------------------------- | ------ | ------ | ---------------- |
| P2-UX-001 | ActionBar: Fold/Call 按钮内显示倒计时秒数            | P2     | ✅ 已实施 | commit a19ded8 — Fold ·{n}s / Call ·{n}s |
| P2-UX-002 | ActionBar: Raise Input 默认值同步 minRaiseTo + focus | P2     | ✅ 已实施 | commit a19ded8 — autoFocus + minRaiseTo sync |
| P2-UX-003 | ActionBar: All-in 独立金色大按钮（全屏可见）         | P2     | ✅ 已实施 | commit a19ded8 — h-14 golden gradient button |
| P2-UX-004 | page.tsx: Emoji 反应飞行轨迹叠加到 GameTable         | P2     | 待实施   | GGPoker 飞行轨迹 |
...
| P0-NOTIFY-MIGRATE | `UserNotificationSettings` 表存在但无迁移脚本 | ✅ 已创建迁移脚本 | `prisma/migrations/202604301000_add_notification_settings/` 已创建；生产执行 `npx prisma migrate deploy` |
